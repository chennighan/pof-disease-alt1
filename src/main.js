...
function detectAlt1() {
  ...
  alt1Status.textContent = `Alt1 detected. Permissions: ${perms.length ? perms.join(", ") : "not installed / not granted yet"}. RS linked: ${window.alt1.rsLinked ? "yes" : "no"}. Size: ${window.alt1.rsWidth || "?"}x${window.alt1.rsHeight || "?"}.`;
  setDefaultScanBox();
}
...
function scanScreenOnce({ quiet = false } = {}) {
  ...
  if (visualCapture) {
    ...
    if (!box) {
      scanStatus.textContent = `Watching for the animal dialogue. Tick ${scanTicks}. Collected ${observedSymptoms.size}/4 checks.`;
      return;
    }
    applyScanBox(box);
